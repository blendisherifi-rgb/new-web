 ...
 *
 * AP Automation background: `sectionBackground: <schemaField>` — JSON key is always `sectionBackground`.
 * Default schema field `sectionbackground` (ACF name). Override with env AP_AUTOMATION_BG_SCHEMA_FIELD.
 */
...
/** WPGraphQL field name for ACF “Section background” (default `sectionbackground`). Set `AP_AUTOMATION_BG_SCHEMA_FIELD=sectionBackground` if your schema uses camelCase only. */
const AP_AUTOMATION_BG_SCHEMA_FIELD = (() => {
  const raw = process.env.AP_AUTOMATION_BG_SCHEMA_FIELD ?? "sectionbackground";
  return /^[a-zA-Z][a-zA-Z0-9_]*$/.test(raw) ? raw : "sectionbackground";
})();
...
function buildFragment(): string {
  const sectionsSelection = `
    ...
    ... on ${ApAutomationLayout} {
      overline
      sectionBackground: ${AP_AUTOMATION_BG_SCHEMA_FIELD}
      headingLine1
      headingLine2
      image { node { sourceUrl altText } }
...
function buildFragmentResilient(): string {
  const sectionsSelection = `
    ...
    ... on ${ApAutomationLayout} { overline sectionBackground: ${AP_AUTOMATION_BG_SCHEMA_FIELD} headingLine1 headingLine2 image { node { sourceUrl altText } } softcoApImage { node { sourceUrl altText } } body ctaLabel ctaHref gartnerLogo { node { sourceUrl altText } } endorsementText metrics { value label } }
